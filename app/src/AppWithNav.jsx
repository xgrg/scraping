...
// ── Resolve URL params once at startup ──────────────────────────────────────
// c param wins over d param (they are mutually exclusive).
// Returns { fromClub, fromDept } — at most one is set.
function resolveUrlParams() {
  ...
  if (cParam) {
    // Find which department this club belongs to
    for (const [deptCode, clubs] of Object.entries(CLUBS)) {
      if (clubs[cParam]) {
        return { fromClub: cParam, fromDept: deptCode };
      }
    }
    // club not found in data — ignore
  }

  if (dParam) {
    return { fromClub: "", fromDept: dParam };
  }

  return { fromClub: "", fromDept: "" };
}

// ── Page Stats ──────────────────────────────────────────────────────────────
function PageStats({ initialClubId }) {
  // Resolve URL params once; never re-read them during the session
  const [{ fromClub, fromDept }] = useState(() => resolveUrlParams());

  // fromClub (c param) takes priority over initialClubId (?id= legacy param)
  const lockedClub = fromClub || initialClubId;
  const lockedDept = fromDept; // set when only ?d= is present

  const [departmentId, setDepartmentId] = useState(lockedDept || "");
  const [clubId, setClubId] = useState(lockedClub || "");
  const [status, setStatus] = useState("idle");
  const [step, setStep] = useState(0);
  const [progress, setProgress] = useState(0);
  const [result, setResult] = useState(null);
  const [errorMsg, setErrorMsg] = useState("");
  const [autoLaunch, setAutoLaunch] = useState(!!lockedClub);

  // Reset club when user manually changes department — but only when
  // neither selector is locked (i.e. no URL params).
  const deptIsLocked = !!lockedDept || !!lockedClub;
  const [prevDept, setPrevDept] = useState(departmentId);
  useEffect(() => {
    if (!deptIsLocked && departmentId !== prevDept) {
      setClubId("");
    }
    setPrevDept(departmentId);
  ...
  function handleReset() {
    setStatus("idle");
    // Only reset selectors when they weren't locked by URL params
    if (!lockedDept && !lockedClub) setDepartmentId("");
    if (!lockedClub) setClubId("");
    setResult(null);
    setErrorMsg("");
    setProgress(0);
    setStep(0);
  ...
  }, [autoLaunch, clubId]); // eslint-disable-line react-hooks/exhaustive-deps

  // Whether each selector is visible
  const showDeptSelector = !lockedDept && !lockedClub;
  const showClubSelector = !lockedClub;
  ...
  return (
    <div style={s.page}>
      ...
      {status === "idle" && (
        <div style={s.card}>
          {/* Département Selector — hidden when locked by URL */}
          {showDeptSelector && (
            <>
          ...
          {/* Club Selector — hidden when locked by URL (c param) */}
          {showClubSelector && (
            <>
              <label
                style={{ ...s.label, marginTop: showDeptSelector ? "1rem" : 0 }}
                htmlFor="club-select"
              >
                Club
              </label>
        ...
        <div>
          <div style={s.resultsHeader}>
            <div style={s.clubBadge}>
              ...
              {CLUBS[departmentId]?.[result.club_id] ?? result.club_name}
            </div>
            <a
...
function PagePoster() {
  ...
  const lockedClub = fromClub;
  const lockedDept = fromDept;
  ...
  // Reset club when user manually changes department — but only when unlocked
  const deptIsLocked = !!lockedDept || !!lockedClub;
  const [prevDept, setPrevDept] = useState(departmentId);
  useEffect(() => {
    if (!deptIsLocked && departmentId !== prevDept) {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        {/* Département — hidden when locked */}
        {showDeptSelector && (
          <div style={s.field}>
        ...
        {/* Club — hidden when locked by c param */}
        {showClubSelector && (
          <div style={s.field}>
      ...
      {status === "loading" && (
        <div style={{ ...s.card, textAlign: "center" }}>
          ...
          <p style={s.loadingSub}>
            Collecte des matchs en cours, merci de patienter.
          </p>
        </div>
      )}
      ...
      {status === "done" && poster && (
        <div style={s.card}>
          <div style={s.resultsHeader}>
            <div style={s.clubBadge}>
              <span style={s.clubDot} />
              {CLUBS[departmentId]?.[clubId]} — J{journee} Ph.{phase}
            </div>
...
export default function App() {
  ...
  const [tab, setTab] = useState(
    query.get("tab") === "poster" ? "poster" : "stats",
  );
    ...
    <div style={s.shell}>
      ...
      <nav style={s.nav}>
        <button style={s.navBtn} onClick={() => setTab("stats")}>
          ...
          <span
            style={{ ...s.navLabel, ...(tab === "stats" ? s.navLabelOn : {}) }}
          >
            Stats
          </span>
        </button>
        <button style={s.navBtn} onClick={() => setTab("poster")}>
          ...
          <span
            style={{ ...s.navLabel, ...(tab === "poster" ? s.navLabelOn : {}) }}
          >
            Affiche
          </span>
        </button>
